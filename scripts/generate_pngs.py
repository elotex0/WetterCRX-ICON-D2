...
# ------------------------------
# Bounding Box Deutschland (fix)
# ------------------------------
extent = [-10, 36, 40, 71]  # lon_min, lon_max, lat_min, lat_max
...
# Feste Kartendomäne (Deutschland-Box) einmalig nach EPSG:3857 (Meter)
...
# nötig, da das PNG bereits auf genau dieses Raster projiziert wurde).
...
def _append_raw_chunk(path, chunk_type: bytes, payload: bytes):
    """Fügt einen rohen (nicht-text) PNG-Chunk direkt vor IEND ein."""
    with open(path, "rb") as f:
        content = f.read()
    iend_start = content.rfind(b"\x00\x00\x00\x00IEND")  # Länge(4)+Typ(IEND) des IEND-Chunks
    length = struct.pack(">I", len(payload))
    crc = struct.pack(">I", zlib.crc32(chunk_type + payload) & 0xFFFFFFFF)
    new_chunk = length + chunk_type + payload + crc
    with open(path, "wb") as f:
        f.write(content[:iend_start] + new_chunk + content[iend_start:])

def save_transparent_png(data, cmap, norm, out_path, raw_data=None, var_type=None, extra_meta=None):
    rgba = data_to_rgba(data, cmap, norm)
    img = Image.fromarray(rgba[::-1, :, :], mode="RGBA")

    if raw_data is not None:
        decimals = VALUE_DECIMALS.get(var_type, 1)
        scale = 10 ** decimals

        scaled = np.round(raw_data * scale)
        scaled = np.where(np.isfinite(raw_data), scaled, NODATA_INT16)
        scaled = np.clip(scaled, -32000, 32000).astype(np.int16)
        scaled = scaled[::-1, :]  # gleiche Ausrichtung wie rgba

        payload = zlib.compress(scaled.tobytes(), level=6)

        meta = {"width": scaled.shape[1], "height": scaled.shape[0],
                "nodata": NODATA_INT16, "scale": scale}
        if extra_meta:
            meta.update(extra_meta)

        info = PngInfo()
        info.add_text("raw_meta", json.dumps(meta))  # bleibt klein, kein Problem
        img.save(out_path, pnginfo=info)

        _append_raw_chunk(out_path, b"raWv", payload)
    else:
        img.save(out_path)
...
for filename in all_files_global:
    ...
    if lon.ndim == 1 and lat.ndim == 1 and data.ndim == 2:
        try:
            method = "nearest" if var_type == "ww" else "linear"
            interp_func = RegularGridInterpolator(
                (lat[::-1], lon),
                data[::-1, :],
                method=method,
    ...
    # Zeile 0 = Süden. Für save_transparent_png reicht das - dort wird
    ...
    # ------------------------------
    # Transparentes PNG speichern
    # ------------------------------
    outname = f"{var_type}_{valid_time_local:%Y%m%d_%H%M}.png"
    out_path = os.path.join(output_dir, outname)
    extra_meta = {"codes": [int(c) for c in codes]} if var_type == "ww" else None
    save_transparent_png(render_data_merc, cmap, norm, out_path,
                        raw_data=render_data_merc, var_type=var_type, extra_meta=extra_meta)
